...
const thirdPartyHost = process.env.HEROKU_APP_NAME ? `${process.env.HEROKU_APP_NAME}.herokuapp.com` : 'localhost:5000';
...
module.exports = (article, options) => Object.assign(article, {
	...
	AUTH_AUTHORIZATION_URL: options.accessMocked ? `//${options.host}/amp-access-mock/access?` : `https://${liveAccessHost}/amp-access?`,

	AUTH_PINGBACK_URL: options.accessMocked ? `//${options.host}/amp-access-mock/pingback?` : `https://${liveAccessHost}/amp-pingback?`,

	AUTH_LOGIN_URL: options.accessMocked ? `//${options.host}/amp-access-mock/login?` : 'https://accounts.ft.com/login?',

	AUTH_LOGOUT_URL: options.accessMocked ? `//${options.host}/amp-access-mock/logout?` : `https://${liveAccessHost}/amp-logout?`,
