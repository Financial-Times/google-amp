...
}).then(({ css }) => postCss([autoprefixer({ browsers: 'last 2 versions' }), removeImportant, inlineSvg, discardEmpty].concat(html ? [removeUnused({ html })] : [])).process(css)).then(compiled => compiled.toString());

const getCSSEntries = () => fs.readdir(scssPath).then(files => files.filter(file => file.endsWith('.scss') && !file.startsWith('_')).map(file => path.basename(file, '.scss')));
...
const getFeatureHTML = entry => entry in featureTemplate ? handlebars.standalone().then(hbs => hbs.renderView(featureTemplate[entry], Object.assign({
	body: ''
}, enableAllFlags))) : Promise.resolve(false);

const getFeatureCSS = getCSS => () => getCSSEntries().then(entries => promiseAllObject(entries.reduce((css, entry) => Object.assign(css, {
...
const writeFeatureCSS = () => mkdirp(cssPath).then(compileFeatureCSS).then(features => Promise.all(Object.keys(features).map(feature => fs.writeFile(cssFile(feature), features[feature], 'utf8'))));

module.exports = (article, options) => {
	const start = Date.now();

	return Promise.resolve(options.production ? cacheIf.always(readFeatureCSS) : compileFeatureCSS()).then(features => {
		...
		const related = article.moreOns && !!article.moreOns.length || article.storyPackage && !!article.storyPackage.length || article.relatedContent && !!article.relatedContent.length;
		...
		if (css.length > 45000) {
			...
			const err = new (exceeding ? Error : Warning)(`${bundleSize}, ${exceeding ? 'exceeding' : 'approaching'} ${ampLimit}.`);
