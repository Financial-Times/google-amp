...
const fetchSlideshow = uuid => fetch(`https://api.ft.com/content/items/v1/${uuid}?apiKey=${process.env.API_V1_KEY}`).then(fetchres.json).then(data => {
	if (data && data.item && data.item.assets && data.item.assets[0] && data.item.assets[0].type === 'slideshow') {
		return {
			uuid,
...
module.exports = (article, options) => {
	...
	return slideshowTransform(article.bodyHTML).then(bodyHTML => {
