...
article.authorConcepts && article.authorConcepts.length ? article.authorConcepts.join(', ') : (article.byline || '').replace(/^by\s+/i, '');

const getByline = article => (article.authorConcepts || []).reduce((byline, author) => byline.replace(author.prefLabel, `
		<a
...
const assembleArticle = (uuid, options) => {
	...
	return nEsClient.get(uuid).then(response => {
		if (response && (!response.originatingParty || response.originatingParty === 'FT') && (!response.type || response.type === 'article')) {
			return response;
		}
		...
		throw err;
	}).then(article => articleFlags(article, options)).then(article => {
	...
	.then(article => Promise.all([transformArticle(article, options), addStoryPackage(article, options), addMoreOns(article, options)])

	// Second phase: transforms which rely on first phase fetches
	.then(() => Promise.all([extraArticleData(article, options)]))
...
module.exports.render = (uuid, options) => assembleArticle(uuid, options).then(article => handlebars.standalone().then(hbs => hbs.renderView('article', Object.assign({ layout: 'layout' }, article))));
