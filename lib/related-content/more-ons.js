...
const addArticles = annotation => nEsClient.search({
	...
	size: moreOnCount * 2,
	_source: ['id', 'title', 'metadata', 'standfirst', 'mainImage', 'publishedDate']
}).then(res => res.filter(article => article.title).map(article => ({
...
}))).then(articles => Object.assign(annotation, { articles })).catch(error => Object.assign(annotation, { error }));

const getMoreOnTags = content => {
	const moreOnTags = [];

	const about = content.annotations.find(annotation => annotation.predicate === 'http://www.ft.com/ontology/annotation/about');
	const primarilyClassifiedBy = content.annotations.find(annotation => annotation.predicate === 'http://www.ft.com/ontology/classification/isPrimarilyClassifiedBy');
	const brand = content.annotations.find(annotation => annotation.types && annotation.types.includes('http://www.ft.com/ontology/product/Brand'));
...
module.exports = (article, options) => Promise.all(getMoreOnTags(article).map(addArticles)).then(moreOns => {
	article.moreOns = moreOns.filter(moreOn => {
		if (moreOn.error) {
			if (options.raven) {
